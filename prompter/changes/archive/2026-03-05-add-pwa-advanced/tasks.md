...
- [x] 1.1 Verify `POST /pos/orders` returns structured JSON error responses (not redirects)
      when validation fails — required for the client-side queue to handle rejections

## 2. Offline Sync Queue — Frontend
- [x] 2.1 Add `idb` npm package to `package.json`
- [x] 2.2 Create `resources/js/hooks/useOfflineQueue.ts`:
...
- [x] 2.3 Update `resources/js/Pages/POS/Index.tsx` (or equivalent POS page):
...
- [x] 2.4 Create `resources/js/Components/OfflineSyncBadge.tsx`:
...
- [x] 3.1 Migrate `vite-plugin-pwa` from `generateSW` to `injectManifest` strategy
      to allow custom service worker code
- [x] 3.2 Create `resources/js/sw.ts` (custom service worker source):
...
- [x] 3.3 Update `vite.config.js` to point `injectManifest.swSrc` to `resources/js/sw.ts`

## 4. Shift Close Guard
- [x] 4.1 Update `resources/js/Pages/POS/Shifts/Close.tsx` (or shift close form):
...
- [x] 5.1 Create migration `create_inventory_transfers_table`:
...
- [x] 5.2 Create `app/Models/InventoryTransfer.php` with `$fillable`, `casts()`, and
      relationships: `fromBranch`, `toBranch`, `ingredient`, `requestedBy`, `approvedBy`
- [x] 5.3 Create `app/Http/Controllers/Admin/InventoryTransferController.php`:
...
- [x] 5.4 Add routes in `routes/web.php` under `admin` group:
...
- [x] 6.1 Create `resources/js/Pages/Admin/InventoryTransfers/Index.tsx` — paginated
      transfer list with status badges and approve/reject action buttons
- [x] 6.2 Create `resources/js/Pages/Admin/InventoryTransfers/Create.tsx` — form with
      from_branch, to_branch, ingredient, quantity, notes fields
- [x] 6.3 Add `InventoryTransfer` TypeScript interface to `types/index.d.ts`
- [x] 6.4 Add "Inventory Transfers" nav link in admin navigation

## 7. Push Notifications — Backend
- [x] 7.1 Add `minishlink/web-push` (or equivalent) to `composer.json`
- [x] 7.2 Create migration `create_push_subscriptions_table`:
      `id, user_id (FK→users), endpoint (text unique), public_key (text),
       auth_token (text), timestamps`
- [x] 7.3 Create `app/Models/PushSubscription.php` with `$fillable` and `user()` relationship
- [x] 7.4 Create `app/Http/Controllers/POS/PushSubscriptionController.php`:
...
- [x] 7.6 Add VAPID keys to `.env` and `.env.example`:
      `VAPID_PUBLIC_KEY=`, `VAPID_SUBJECT=`, `VAPID_PRIVATE_KEY=`
- [x] 7.7 Create `app/Notifications/LowStockAlert.php` (via custom WebPush channel)
- [x] 7.8 Create `app/Notifications/InventoryTransferStatusUpdate.php`
- [x] 7.9 Create `app/Notifications/DailySalesSummary.php`
- [x] 7.10 Hook `LowStockAlert` into `OrderController.store()` post-order-creation for
        ingredients that cross the restock threshold
- [x] 7.11 Hook `InventoryTransferStatusUpdate` into `InventoryTransferController.approve/reject`
- [x] 7.12 Create a scheduled Artisan command `app:send-daily-summary` and register in
        `console.php` to run at 23:59 Asia/Jakarta daily

## 8. Push Notifications — Frontend
- [x] 8.1 Create `resources/js/Components/PushNotificationPrompt.tsx`:
...
- [x] 8.2 Mount `<PushNotificationPrompt />` in `AuthenticatedLayout.tsx`
- [x] 8.3 Add `push` event handler to `resources/js/sw.ts` (the custom service worker):
      - Parse JSON payload from `event.data.json()`
      - Call `self.registration.showNotification(title, { body, icon, data: { url } })`
- [x] 8.4 Add `notificationclick` event handler in `sw.ts`:
